...
class Deployer {
  ...
  async deployEventsHub(registryAddr) {
    ...
    let proxy = await contracts.EventsHubProxy.new(utils.ZeroAddress);
  ...
  buildStakeManagerObject(stakeManager, governance) {
    ...
    stakeManager.updateCheckPointBlockInterval = (val) => {
      return governance.update(
        stakeManager.address,
        stakeManager.contract.methods
          .updateCheckPointBlockInterval(val)
          .encodeABI()
      );
    };
    ...
    stakeManager.updateCheckpointRewardParams = (val1, val2, val3) => {
      return governance.update(
        stakeManager.address,
        stakeManager.contract.methods
          .updateCheckpointRewardParams(val1, val2, val3)
          .encodeABI()
      );
    };
  }
  ...
  async deployGovernance() {
    ...
    this.governanceProxy = await contracts.GovernanceProxy.new(
      governance.address
    );
  ...
  async deployDrainable() {
    ...
    this.drainable = await contracts.Drainable.at(
      this.depositManagerProxy.address
    );
  ...
  async deployErc20Predicate(burnOnly) {
    ...
    const ERC20Predicate = await predicate.new(...args);
    await this.governance.update(
      this.registry.address,
      this.registry.contract.methods
        .addErc20Predicate(ERC20Predicate.address)
        .encodeABI()
    );
  ...
  async deployErc721Predicate(burnOnly) {
    ...
    await this.governance.update(
      this.registry.address,
      this.registry.contract.methods
        .addErc721Predicate(ERC721Predicate.address)
        .encodeABI()
    );
  ...
  async deployMintableErc721Predicate() {
    ...
    await this.addPredicate(predicate.address, 3 /* Type.Custom */);
    return predicate;
  }
  ...
  async deployMarketplacePredicate() {
    ...
    await this.addPredicate(MarketplacePredicate.address, 3 /* Type.Custom */);
    return MarketplacePredicate;
  }

  async deployTransferWithSigPredicate() {
    const TransferWithSigPredicate =
      await contracts.TransferWithSigPredicate.new(
        this.rootChain.address,
        this.withdrawManagerProxy.address,
        this.registry.address
      );
  ...
  mapToken(rootTokenAddress, childTokenAddress, isERC721 = false) {
    return this.governance.update(
      this.registry.address,
      this.registry.contract.methods
        .mapToken(rootTokenAddress, childTokenAddress, isERC721)
        .encodeABI()
    );
  }
  ...
  async deployChildErc20(owner, options = { mapToken: true }) {
    ...
    if (!this.childERC20Proxified) {
      this.childERC20Proxified = await contracts.ChildERC20Proxified.new({
        gas: 20000000,
      });
    }
    const childTokenProxy = await contracts.ChildTokenProxy.new(
      this.childERC20Proxified.address
    );
    const childToken = await contracts.ChildERC20Proxified.at(
      childTokenProxy.address
    );
  ...
  async deployMaticToken() {
    ...
    const rootERC20 = await this.deployTestErc20({
      mapToken: true,
      childTokenAdress: childToken.address,
    });
    ...
    await this.globalMatic.childToken.deposit(
      childToken.address,
      web3.utils.toBN(100).mul(utils.scalingFactor)
    );
  ...
  async deployChildErc721(owner, options = { mapToken: true }) {
    ...
    if (!this.childERC721Proxified) {
      this.childERC721Proxified = await contracts.ChildERC721Proxified.new({
        gas: 20000000,
      });
    }
    const childTokenProxy = await contracts.ChildTokenProxy.new(
      this.childERC721Proxified.address
    );
    const childErc721 = await contracts.ChildERC721Proxified.at(
      childTokenProxy.address
    );
  ...
  async initializeChildChain(owner, options = { updateRegistry: true }) {
    ...
    if (!this.globalMatic) {
      ...
      this.globalMatic = {
        childToken: await contracts.MRC20.at(utils.ChildMaticTokenAddress),
      };
