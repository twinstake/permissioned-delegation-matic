...
function shouldWithdrawReward({
  ...
  if (checkBalance) {
    it("must have updated balance", async function () {
      ...
      assertBigNumberEquality(
        balance,
        new BN(initialBalance).add(new BN(reward))
      );
    });
  }

  it("must have liquid rewards == 0", async function () {
    let rewards = await this.validatorContract.getLiquidRewards(
      user || this.user
    );
  ...
  it("must have correct initialRewardPerShare", async function () {
    ...
    const userRewardPerShare =
      await this.validatorContract.initalRewardPerShare(user || this.user);
...
contract("ValidatorShare", async function () {
  ...
  async function slash(
    ...
    for (const slash of slashes) {
      slashingInfoList.push([
        parseInt(slash.validator),
        new BN(slash.amount),
        "0x0",
      ]);
    }

    return updateSlashedAmounts(
      validators,
      proposer,
      nonce,
      slashingInfoList,
      this.slashingManager
    );
  }
  ...
  describe("buyVoucher", function () {
    ...
    describe("when validator turns off delegation", function () {
      ...
      it("reverts", async function () {
        await expectRevert(
          buyVoucher(this.validatorContract, toWei("150"), this.alice),
    ...
    describe("when staking manager delegation is disabled", function () {
      ...
      before("disable delegation", async function () {
        await this.governance.update(
          this.stakeManager.address,
          this.stakeManager.contract.methods
    ...
    describe("when validator unstaked", function () {
      ...
      it("reverts", async function () {
        await expectRevert(
          buyVoucher(this.validatorContract, new BN(toWei("100")), this.alice),
          "locked"
        );
      });
    });
  ...
  describe("exchangeRate", function () {
    describe("when Alice purchases voucher 2 times, 1 epoch between", function () {
      ...
      it("exchange rate must be correct", async function () {
        assertBigNumberEquality(
          await this.validatorContract.exchangeRate(),
          ...
          [
            {
              ...
              amount: await this.stakeManager.currentValidatorSetTotalStake(),
            },
          ],
          [this.validatorUser],
          this.validatorUser,
  ...
  describe("sellVoucher", function () {
    ...
    function testSellVoucherNew({
      ...
      if (minClaimAmount) {
        it("must sell voucher with slippage", async function () {
          this.receipt = await sellVoucherNew(
            this.validatorContract,
            user,
            minClaimAmount
          );
    ...
    function testSellVoucher({
      ...
      if (minClaimAmount) {
        it("must sell voucher with slippage", async function () {
          this.receipt = await sellVoucher(
            this.validatorContract,
            user,
            minClaimAmount
          );
    ...
    describe("when all tokens are slashed", function () {
      ...
      it("reverts", async function () {
        await expectRevert(
          sellVoucher(this.validatorContract, Alice, "0"),
          "Too much requested"
        );
      });
    });
    ...
    describe("when Alice sells with claimAmount greater than expected", function () {
      ...
      it("reverts", async function () {
        ...
        await expectRevert(
          this.validatorContract.sellVoucher(toWei("100.00001"), maxShares, {
            from: Alice,
          }),
          "Too much requested"
        );
      ...
      describe("new API", function () {
        ...
        describe("when Alice is slashed by 50%", function () {
          ...
          before(async function () {
            await slash.call(
              this,
              [{ validator: this.validatorId, amount: toWei("100") }],
              [this.validatorUser],
              this.validatorUser,
              1
            );
          ...
          describe("when Alice sells 50%, after 1 epoch, within withdrawal delay", function () {
            ...
            it("unbond epoch must be set to current epoch", async function () {
              ...
              assertBigNumberEquality(
                ...
                await this.stakeManager.currentEpoch()
              );
            });
          });
        });
  ...
  describe("withdrawRewards", function () {
    ...
    function testWithdraw({ label, user, expectedReward, initialBalance }) {
      describe(`when ${label} withdraws`, function () {
        if (expectedReward.toString() === "0") {
          it("reverts", async function () {
            await expectRevert(
              this.validatorContract.withdrawRewards({
                from: user,
              }),
      ...
      describe(`${label} buyVoucher for ${amount.toString()} wei`, function () {
        ...
        it("must have correct initalRewardPerShare", async function () {
          const currentRewardPerShare =
            await this.validatorContract.getRewardPerShare();
          const userRewardPerShare =
            await this.validatorContract.initalRewardPerShare(user);
          assertBigNumberEquality(currentRewardPerShare, userRewardPerShare);
        });
      });
    }
      ...
      describe("checkpoints", function () {
        it(`${checkpoints} more checkpoint(s)`, async function () {
          ...
          for (const user in delegators) {
            ...
            totalDelegatorRewardsReceived =
              totalDelegatorRewardsReceived.add(rewards);
          }
        });
      });
    }
    ...
    function testAllRewardsReceived({ validatorReward, totalExpectedRewards }) {
      ...
      describe("total rewards", function () {
        it(`validator rewards == ${validatorReward.toString()}`, async function () {
          assertBigNumberEquality(
            await getValidatorReward.call(this),
            validatorReward
          );
        });
        ...
        it(`all expected rewards should be ${totalExpectedRewards.toString()}`, async function () {
          ...
          assertBigNumberEquality(
            validatorRewards.add(totalDelegatorRewardsReceived),
        ...
        it(`total received rewards must be correct`, async function () {
          const validatorRewards = await getValidatorReward.call(this);
          const totalReceived = validatorRewards.add(
            totalDelegatorRewardsReceived
          );
    ...
    function runWithdrawRewardsTest(timeline) {
      ...
      before(async function () {
        ...
        totalDelegatorRewardsReceived = new BN(0);
        this.initialStakeTokenBalance = await this.stakeToken.balanceOf(
          this.stakeManager.address
        );
      });
    ...
    describe("when Alice purchases voucher after checkpoint", function () {
      runWithdrawRewardsTest([
        ...
        {
          allRewards: {
            validatorReward: toWei("9000"),
            totalExpectedRewards: toWei("9000"),
          },
        },
      ]);
    });
      ...
      runWithdrawRewardsTest([
        ...
        { checkpoints: 1 },
        {
          withdraw: {
            user: Alice,
            label: "Alice",
            expectedReward: toWei("4500"),
          },
        },
        {
          allRewards: {
            validatorReward: toWei("4500"),
            totalExpectedRewards: toWei("9000"),
          },
        },
      ]);
        ...
        describe("when Alice and Bob withdraw rewards after", function () {
          runWithdrawRewardsTest([
            ...
            },
            {
              allRewards: {
                validatorReward: toWei("4500"),
                totalExpectedRewards: toWei("18000"),
              },
            },
          ]);
        });
      ...
      runWithdrawRewardsTest([
        ...
        { checkpoints: 1 },
        {
          liquidRewards: {
            user: Alice,
            label: "Alice",
            expectedReward: "5785714285714285714285",
          },
        },
        {
          liquidRewards: {
            user: Bob,
            label: "Bob",
            expectedReward: "6428571428571428571428",
          },
        },
        {
        ...
        { checkpoints: 1 },
        {
          liquidRewards: {
            user: Alice,
            label: "Alice",
            expectedReward: "6315126050420168067226",
          },
        },
        {
          liquidRewards: {
            user: Bob,
            label: "Bob",
            expectedReward: "9075630252100840336133",
          },
        },
        {
          liquidRewards: {
            user: Carol,
            label: "Carol",
            expectedReward: "5294117647058823529411",
          },
        },
        { slash: { amount: new BN(toWei("167")), nonce: 2 } },
        {
          stake: {
            user: Eve,
            label: "Eve",
            amount: new BN(toWei("500")),
            initialBalance: new BN(1),
          },
        },
        { checkpoints: 1 },
        {
          withdraw: {
            user: Alice,
            label: "Alice",
            expectedReward: "6620779672815871910894",
          },
        },
        {
          withdraw: {
            user: Bob,
            label: "Bob",
            expectedReward: "10603898364079359554473",
          },
        },
        {
          withdraw: {
            user: Carol,
            label: "Carol",
            expectedReward: "8350653871015861966090",
          },
        },
        {
          withdraw: {
            user: Eve,
            label: "Eve",
            expectedReward: "3803888419273034657649",
            initialBalance: new BN(1),
          },
        },
        {
          allRewards: {
            validatorReward: "6620779672815871910888",
            totalExpectedRewards: "35999999999999999999994",
          },
        },
      ]);
    });
      ...
      runWithdrawRewardsTest([
        ...
        { checkpoints: 1 },
        {
          withdraw: {
            user: Alice,
            label: "Alice",
            expectedReward: "6471712628713457213871",
          },
        },
        {
          withdraw: {
            user: Bob,
            label: "Bob",
            expectedReward: "9858563143567286069357",
          },
        },
        {
          withdraw: {
            user: Carol,
            label: "Carol",
            expectedReward: "6859983429991714995859",
          },
        },
        {
          withdraw: {
            user: Eve,
            label: "Eve",
            expectedReward: "6338028169014084507041",
            initialBalance: new BN(1),
          },
        },
        {
          allRewards: {
            validatorReward: "6471712628713457213867",
            totalExpectedRewards: "35999999999999999999995",
          },
        },
      ]);
    });
    ...
    describe("when not enough rewards", function () {
      ...
      it("reverts", async function () {
        await expectRevert(
          this.validatorContract.withdrawRewards({ from: Alice }),
          "Too small rewards amount"
        );
      });
    });
  ...
  describe("restake", function () {
    ...
    describe("when no liquid rewards", function () {
      ...
      it("reverts", async function () {
        await expectRevert(
          this.validatorContract.restake({ from: this.user }),
          "Too small rewards to restake"
        );
      });
    });
  ...
  describe("unstakeClaimTokens", function () {
    ...
    describe("when Alice unstakes right after voucher sell", function () {
      ...
      before("checkpoint", async function () {
        ...
        let exitEpoch = currentEpoch.add(
          await this.stakeManager.WITHDRAWAL_DELAY()
        );
    ...
    describe("when Alice claims too early", function () {
      ...
      it("reverts", async function () {
        await expectRevert(
          this.validatorContract.unstakeClaimTokens({
            from: this.user,
          }),
    ...
    describe("new API", function () {
      describe("when Alice claims 2 seperate unstakes (1 epoch between unstakes)", function () {
        ...
        it("must claim 1st unstake", async function () {
          this.receipt = await this.validatorContract.unstakeClaimTokens_new(
            "1",
            {
              from: this.user,
            }
        ...
        it("must have 0 shares", async function () {
          assertBigNumberEquality(
            await this.validatorContract.balanceOf(this.user),
            "0"
          );
        });
      });
  ...
  describe("transfer", function () {
    describe("when Alice has no rewards", function () {
      ...
      before("Alice purchases voucher", async function () {
        await buyVoucher(this.validatorContract, toWei("100"), this.alice);
        initialSharesBalance = await this.validatorContract.balanceOf(
          this.alice
        );
      });

      it("must Transfer shares", async function () {
        await this.validatorContract.transfer(this.bob, initialSharesBalance, {
          from: this.alice,
        });
      });
      ...
      it("Alice must have 0 shares", async function () {
        assertBigNumberEquality(
          await this.validatorContract.balanceOf(this.alice),
          "0"
        );
      });
    ...
    describe("when Alice and Bob has unclaimed rewards", function () {
      ...
      before(
        "Alice and Bob purchases voucher, checkpoint is commited",
        async function () {
          await buyVoucher(
            this.validatorContract,
            ValidatorDefaultStake,
            this.alice
          );
          await buyVoucher(
            this.validatorContract,
          ...
          initialAliceSharesBalance = await this.validatorContract.balanceOf(
            this.alice
          );
          initialBobSharesBalance = await this.validatorContract.balanceOf(
            this.bob
          );

          initialAliceMaticBalance = await this.stakeToken.balanceOf(
            this.alice
          );
      ...
      it("must Transfer shares", async function () {
        this.receipt = await this.validatorContract.transfer(
          this.bob,
          initialAliceSharesBalance,
          { from: this.alice }
        );
      });
      ...
      it("Alice must claim 3000 matic", async function () {
        assertBigNumberEquality(
          ...
          new BN(initialAliceMaticBalance).add(new BN(toWei("3000")))
        );
      });

      it("Alice must have 0 liquid rewards", async function () {
        assertBigNumberEquality(
          await this.validatorContract.getLiquidRewards(this.alice),
          "0"
        );
      });
      ...
      it("Bob must claim 3000 matic", async function () {
        assertBigNumberEquality(
          ...
          new BN(initialBobMaticBalance).add(new BN(toWei("3000")))
        );
      });
    ...
    describe("when transfer to 0x0 address", function () {
      ...
      it("reverts", async function () {
        await expectRevert.unspecified(
          this.validatorContract.transfer(ZeroAddr, initialAliceSharesBalance, {
            from: this.alice,
          })
        );
      });
